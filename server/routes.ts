...
export async function registerRoutes(
  ...
  app.delete("/api/job-milestones/:id", isAuthenticated, async (req, res) => {
    try {
      // Check if any PC items are assigned to this milestone
      ...
      const assignedItems = pcItems.filter(item => (item as any).milestoneId === req.params.id);
      
      if (assignedItems.length > 0) {
        // Reassign tasks to no milestone before deleting
